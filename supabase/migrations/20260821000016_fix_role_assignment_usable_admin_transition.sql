-- Migration: Correct Destructive Usable-ADMIN Transition Detection for Role Assignment RPC
-- Spec Reference: STAFF-GOV1E1-FIX1 Database Governance Invariant
...
DECLARE
    ...
    v_currently_usable_admin BOOLEAN := FALSE;
...
BEGIN
    ...
    -- 2a. Lock target public.staff row and re-read active & user_id state under lock
    SELECT is_active, user_id
    INTO v_staff_is_active, v_staff_user_id
    FROM public.staff
    WHERE id = p_staff_id
    FOR UPDATE;
    ...
    -- 2b. Lock target public.clinics row and re-read clinic active state under lock
    ...
    -- 4. Determine if target staff currently contributes a USABLE ADMIN at this clinic
    -- Invariant: Requires clinic active AND staff active AND usable auth (user_id NOT NULL) AND membership active AND ADMIN role
    IF v_membership_id IS NOT NULL
       AND v_membership_is_active = TRUE
       AND v_clinic_is_active = TRUE
       AND v_staff_is_active = TRUE
    ...
    -- The last-admin check runs ONLY when removing an existing USABLE ADMIN contribution from an active clinic
    IF v_currently_usable_admin = TRUE AND v_new_has_admin = FALSE THEN
